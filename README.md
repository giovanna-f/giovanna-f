# Olá! Eu sou a Giovanna Fernandes 

- Estudante de Sistemas de Informação – Universidade Federal de Uberlândia (UFU)  
- Interesse em desenvolvimento de software e tecnologia  

Este repositório é meu portfólio acadêmico, onde compartilho meus projetos, estudos e aprendizados na área de programação e sistemas.

---

## Objetivo

Desenvolver minha base técnica em programação e construção de sistemas, buscando evolução constante e preparação para o mercado de tecnologia.

---

## Tecnologias e Ferramentas
- HTML e CSS  
- Linguagem C  
- Java e js
- MySQL    
- Excel e Google Sheets
  
---

## Em aprendizado
- Desenvolvimento web  
- Back-end com Java  
- Banco de dados  
- Estrutura de projetos de software
  
---

## Projetos

Aqui você encontrará meus projetos acadêmicos e exercícios práticos.

Em breve adicionarei:
- Projetos em HTML e CSS  
- Exercícios em C e Java  
- Estudos com banco de dados MySQL  

---

## Contato

- LinkedIn: [linkedin.com/in/giovanna-fernandes-974121388](https://www.linkedin.com/in/giovanna-fernandes-974121388)
- GitHub: https://github.com/giovanna-f
- Email: fernandesg.8434@gmail.com

